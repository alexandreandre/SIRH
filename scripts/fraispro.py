...
def scrape_repas(soup) -> dict:
    ...
    print("Recherche de la section 'Repas'...")
...
def scrape_petit_deplacement(soup) -> list:
    ...
    print("Recherche de la section 'Petit déplacement'...")
    ...
    print(f"  - Barème Petit Déplacement trouvé ({len(data)} tranches).")
    return data

def scrape_grand_deplacement(soup) -> dict:
    """Scrape la section des indemnités de grand déplacement."""
    print("Recherche de la section 'Grand déplacement'...")
    ...
    print(f"  - Barème Grand Déplacement trouvé (Métropole: {len(data['metropole'])}, OM1: {len(data['outre_mer_groupe1'])}, OM2: {len(data['outre_mer_groupe2'])}).")
    return data

# --- FONCTION DE SCRAPING CORRIGÉE ---
def scrape_mutation(soup) -> dict:
    """Scrape la section des frais de mutation professionnelle en gérant les onglets."""
    print("Recherche de la section 'Mutation professionnelle'...")
    ...
    # Itérer sur les boutons pour trouver les panneaux correspondants
    ...
    print(f"  - Barème Mutation Professionnelle trouvé : {data}")
    return data

def scrape_mobilite_durable(soup) -> dict:
    """Scrape la section du forfait mobilités durables en gérant les onglets."""
    print("Recherche de la section 'Mobilité durable'...")
...
def scrape_teletravail(soup) -> dict:
    ...
    # Partie 1 : Indemnité forfaitaire (avec onglets)
    ...
    # Partie 2 : Indemnité pour matériel informatique
    ...
    print(f"  - Barème Télétravail trouvé : {data}")
    return data
    
def get_and_update_baremes():
    """Orchestre le scraping et la mise à jour."""
    ...
    # Scraper les données de TOUTES les sections
    repas_data = scrape_repas(soup)
    petit_deplacement_data = scrape_petit_deplacement(soup)
    grand_deplacement_data = scrape_grand_deplacement(soup)
    mutation_data = scrape_mutation(soup)
    mobilite_data = scrape_mobilite_durable(soup)
    teletravail_data = scrape_teletravail(soup) # <-- APPEL AJOUTÉ
    
    # Vérifier que toutes les sections principales ont été trouvées
    if not all([repas_data, petit_deplacement_data, grand_deplacement_data, mutation_data, mobilite_data, teletravail_data]):
        print("\nScraping incomplet, le fichier JSON ne sera pas mis à jour. Vérifiez les sections manquantes.")
        return
    ...
    try:
        ...
        frais_pro['petit_deplacement_bareme'] = petit_deplacement_data
        frais_pro['grand_deplacement'] = grand_deplacement_data
        frais_pro['mutation_professionnelle'] = mutation_data
        frais_pro['mobilite_durable'] = mobilite_data
        frais_pro['teletravail'] = teletravail_data # <-- LIGNE AJOUTÉE
...
if __name__ == "__main__":
    try:
        get_and_update_baremes()
    except Exception as e:
        print(f"ERREUR : Le script principal a échoué. Raison : {e}")